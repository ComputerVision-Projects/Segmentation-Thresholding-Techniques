...
#     def __init__(self, image_path, output_widget):
#         super().__init__()
#         self.cv_image = cv2.imread(image_path)
#         self.region_grower = RegionGrowing(self.cv_image)
#         self.seed_point = None
#         self.segmented_img = None  # mask result
#         self.show_mask = False  # toggle showing the segmentation
#         self.image_path = image_path
#         self.output_widget=output_widget

#         height, width, channel = self.cv_image.shape
#         bytes_per_line = 3 * width
#         qimg = QImage(self.cv_image.data, width, height, bytes_per_line, QImage.Format_BGR888)
...
#                 segmented = self.region_grower.segment_image([(self.seed_point.x(), self.seed_point.y())], threshold=10)
...
#         # else:
#         #     # Display the mask as a QPixmap
#         #     mask = cv2.cvtColor(self.segmented_img, cv2.COLOR_GRAY2BGR)
#         #     height, width, channel = mask.shape
#         #     bytes_per_line = 3 * width
#         #     qimg = QImage(mask.data, width, height, bytes_per_line, QImage.Format_BGR888)
#         #     painter.drawPixmap(0, 0, QPixmap.fromImage(qimg))

#         if self.seed_point and not self.show_mask:
#             pen = QPen(Qt.red)
#             pen.setWidth(5)
#             painter.setPen(pen)
...
#     image_path = "D:\SBME 2026\(3rd year 2nd term) Sixth Term\computer vision\Projects\Segmentation-Thresholding-Techniques\Images\dogg.jpeg"  # Change this to a valid path
